...
class ProcessDefSpec extends FunSpec with BeforeAndAfter {
  ...
  describe("A process with a wait state") {
    it("will have the process variables with which it is started") {
      val processDefActor = system.actorOf(Props(classOf[ProcessDefActor], process), name = "process")
      val actorRefFuture = processDefActor ? StartProcess(Map("foo" -> "bar"))
      val actorRef = Await.result(actorRefFuture.mapTo[ActorRef], 500.millis)
      val variablesFuture = actorRef ? GetVariables
      val variables = Await.result(variablesFuture.mapTo[Map[String, Any]], 500.millis)
